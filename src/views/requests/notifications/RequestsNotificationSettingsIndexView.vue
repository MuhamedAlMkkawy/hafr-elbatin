...
const loadNotifications = async () => {
  ...
  try {
    ...
    const response = await ssNotificationSettingsService.list(params);
...
const goToEdit = (item) => {
  ...
  router.push(
    `${prefix}/requests/settings/notifications/${item.event_key}/edit`,
  );
};
...
const handleToggleStatus = async (item) => {
  ...
  try {
    await ssNotificationSettingsService.toggleStatus(item.id);
