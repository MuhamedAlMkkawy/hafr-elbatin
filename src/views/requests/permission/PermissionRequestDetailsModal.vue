...
const workflowHeaders = computed(() => [
  { key: "action", label: t("permissionRequests.modals.action") },
  { key: "performed_by", label: t("permissionRequests.modals.actionBy") },
  { key: "created_at", label: t("permissionRequests.modals.actionDate") },
]);
...
const formatTime = (timeStr) => {
  ...
  return date.toLocaleTimeString(lang.value === 'ar' ? 'ar-SA' : 'en-US', {
    hour: '2-digit',
    minute: '2-digit',
    hour12: true
  });
...
const isCurrentMonth = computed(() => {
  if (!props.request?.date) return false;
  const now = new Date();
  const itemDate = new Date(props.request.date);
...
const canWithdraw = computed(() => {
  ...
  let statusOk = false;
  if (props.scope === "own") {
    statusOk = props.request.status === "pending";
  } else {
    statusOk = (props.request.status === "pending" || props.request.status === "approved") && !props.request.is_finalized;
  }
...
<template>
  <Modal
    :model-value="show"
    @close="handleClose"
    :title="t('permissionRequests.modals.viewTitle')"
    width="xl"
  >
    <!-- Loading -->
    ...
    <div v-else-if="request" class="space-y-4 py-4">
      <div class="grid grid-cols-1 gap-y-1">
        ...
        <div class="flex items-center pb-3 gap-12">
          <span class="text-[12px] font-[500] text-[#6C737F] w-[170px]">
            {{ t("permissionRequests.fields.reason") }}
          </span>
          <span class="text-[16px] font-[600] text-[#384250]">
            {{ request.reason || "-" }}
          </span>
        ...
        >
          <span class="text-[12px] font-[500] text-red-500 w-[170px]">
            {{ t("permissionRequests.fields.rejectionReason") }}
          </span>
        ...
        <div class="flex items-center pb-3 gap-12">
          ...
          >
            ...
            <span class="text-[14px] font-[500]">
              {{ t("permissionRequests.status." + request.status) }}
            </span>
          </div>
        </div>
      </div>
      ...
      <div v-if="workflowLogs.length > 0" class="mt-4">
        <h3 class="text-[12px] font-[500] text-[#6C737F] mb-4">
          {{ t("permissionRequests.modals.workflow") }}
        </h3>
    ...
    <template #footer>
      <div class="flex flex-wrap justify-end gap-2 w-full">
        ...
        <template v-if="request && (request.status === 'pending' || request.status === 'approved') ">
          <Button
            v-if="canWithdraw && authStore.hasPermission('permission_request.withdraw')"
          ...
          <template v-if="request.status === 'pending' && isCurrentMonth">
            <Button @click="handleReject" class="bg-[#FEF3F2] !text-[#D92D20] hover:bg-[#FDE4E2]" v-if="authStore.hasPermission('permission_request.reject')">
              {{ t("permissionRequests.modals.reject") }}
            </Button>
            <Button variant="primary" @click="handleApprove" v-if="authStore.hasPermission('permission_request.approve')">
